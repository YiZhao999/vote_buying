...
latest_distances = unga_data.groupby('Countryname')[
    ['Distance_Neutral', 'Distance_US', 'Distance_China']].last().reset_index()

print(latest_distances)  # Show sample of the table

unique_countries = unga_data['Countryname'].unique()

for country in unique_countries:
    country_data = unga_data[unga_data['Countryname'] == country]
    plt.figure(figsize=(10, 5))
    plt.plot(country_data['session'], country_data['IdealPointAll'], marker='o', linestyle='-',
             label=f'{country} Ideal Point')
    ...
    plt.title(f'Ideological Trend of {country} in UNGA')
